...
- Eligibility is decided on-chain: a market cap of at least $100,000, priced through Chainlink, held for 24 hours. A token that drops below it before qualifying, or during its battle, is disqualified.
...
- After a battle, the operator publishes the outcome with both scores and commitments to the dataset and the result. A lead of less than one percentage point must be recorded as a draw. The vault's disqualification record comes first: the token that dropped loses, the first of two to drop loses, and two drops in the same second void the battle.
- The guardian can veto a result within 24 hours, and can cancel a booking before it starts, which returns each token's contribution to its pending pot and gives both tokens their battle back.

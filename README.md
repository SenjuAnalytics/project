...
- **Competition share (15%):** routing depends on the token's battle status —
  ...
  - **Disqualified before ever battling:** the **full 15% plus its entire `pendingBattlePot` goes to the treasury**.
  - **Disqualified during a LIVE battle:** the fee still goes **100% to the pot** (the battle keeps running the full 24 hours) and becomes the winner's.
...
- **Eligibility (revised):** the **only** requirement is **market cap ≥ $100,000 USD** (in USD, not the pair asset). A token qualifies once its MC first **closes** at ≥ $100k (measured on the state after a trade, not a momentary spike), computed **on-chain** each trade via **Chainlink** (`AggregatorV3Interface`) price feeds (no off-chain trust). A 24-hour timer then starts; if MC drops below $100k inside that window it is **permanently disqualified**. Each token may battle **at most once in its lifetime**. The old "minimum 100 holders" requirement has been dropped. Feed addresses are configurable (testnet chain 46630 vs mainnet chain 4663). See [`docs/FEE-AND-BATTLE-SPEC.md §2`](docs/FEE-AND-BATTLE-SPEC.md).
...
- The guardian can veto a result within 24 hours; after that the battle settles on its own (the operator service finalizes it, and anyone can), and the first buyback tranche runs in the same transaction. There is no early finalize — every battle runs the full 24 hours. If a token's MC drops below $100k mid-battle it is marked as lost immediately, but the winner is only declared at finalize; the disqualified token's fees still flow 100% to the pot until then.
...
- **Both tokens disqualified during the window:** the one that dropped **first loses**; the token that **survived longer (dropped later) wins** → the full pot buys back and burns the winner. **There is always a winner — this is not a void.** A true void only occurs if both drop in the same second, which is then handled the same as a draw (below). The vault records every drop and only accepts the outcome that record implies, so a posted result can't overturn it.
